...
Etat_e GPIO_Get(Mapping_GPIO_e Pin)
{
	...
	switch(Mapping_GPIO[Pin].GpioMode) {
		...
		case GPIO_Mode_IN:		Etat_GPIO = GPIO_ReadInputDataBit(Peripheral, GPIO_Pin);			break;
		case GPIO_Mode_OUT:		Etat_GPIO = GPIO_ReadOutputDataBit(Peripheral, GPIO_Pin);			break;
		default:																						break;
	}
	...
	switch(Mapping_GPIO[Pin].Inverse) {
		...
		case TRUE:
			switch(State) {
				...
				case ETAT_ACTIF:	if(GPIO_Get(Pin) != ETAT_ACTIF) 	GPIO_ResetBits(Peripheral, GPIO_Pin);	break;
				case ETAT_INACTIF:	if(GPIO_Get(Pin) != ETAT_INACTIF) 	GPIO_SetBits(Peripheral, GPIO_Pin);		break;
				default:																						break;
		...
		case FALSE:
			...
			switch(State) {
				...
				case ETAT_ACTIF:	if(GPIO_Get(Pin) != ETAT_ACTIF) 	GPIO_SetBits(Peripheral, GPIO_Pin);		break;
				case ETAT_INACTIF:	if(GPIO_Get(Pin) != ETAT_INACTIF) 	GPIO_ResetBits(Peripheral, GPIO_Pin);	break;
				default:																						break;
