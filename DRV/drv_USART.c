 ...
 * Private macros
 */
#define	__IsPeriphUSART(Periph)		(		(Periph) != (uint32_t) USART1 	\
						&& 	(Periph) != (uint32_t) USART2	\
						&& 	(Periph) != (uint32_t) USART3	\
						&& 	(Periph) != (uint32_t) USART6		)


#define __USART_GetInitializedState(periph)		 ( __USART[ USART_GetPeriphEnum(periph) ].Initialisazed )
#define __USART_GetBaudRateState(periph)		 ( __USART[ USART_GetPeriphEnum(periph) ].VitesseCom )
#define __USART_GetInterruptState(periph)		 ( __USART[ USART_GetPeriphEnum(periph) ].InterrupState )

#define __USART_SetInitializedState(periph, state)	 ( __USART[ USART_GetPeriphEnum(periph) ].Initialisazed = state )
#define __USART_SetInterruptState(periph, state)	 ( __USART[ USART_GetPeriphEnum(periph) ].InterrupState = state )
#define __USART_SetBaudRateState(periph, speed)	 	 ( __USART[ USART_GetPeriphEnum(periph) ].VitesseCom = speed )
...
Usart_InitPeriph(
	...
	if __IsPeriphUSART(Mapping_GPIO[ID_PinTX].Periph)	return UsartErr_TryingToInitNonUsartPeriph;
	...
	else				return UsartErr_RxOrTxPinInvalid;
